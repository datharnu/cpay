...
export function DashboardCollectionsChart({
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex h-56 items-end justify-between gap-1 overflow-x-auto pb-1 sm:gap-2">
        {months.map((month) => {
          ...
          return (
            ...
            >
              <div className="flex h-44 w-full items-end justify-center gap-1 sm:gap-1.5">
                <div
                  className={`w-[42%] max-w-8 rounded-t-md transition-all ${collectedClass}`}
                  style={{ height: `${collectedPct}%` }}
                  title={
                    month.prepaid
                      ? `Pre-paid ${formatMoney(month.collected)} (overpayment credit)`
                      : `Collected ${formatMoney(month.collected)}`
                  }
                />
                <div
                  className="w-[42%] max-w-8 rounded-t-md border border-slate-500/50 bg-slate-500/35 transition-all"
